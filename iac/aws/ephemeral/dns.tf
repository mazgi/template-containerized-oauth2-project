...
resource "aws_route53_record" "backend" {
  ...
  type    = "CNAME"
  ttl     = 300
  records = [trimprefix(aws_ecs_express_gateway_service.backend.ingress_paths[0].endpoint, "https://")]
}
...
resource "aws_route53_record" "web" {
  ...
  type    = "CNAME"
  ttl     = 300
  records = [trimprefix(aws_ecs_express_gateway_service.web.ingress_paths[0].endpoint, "https://")]
}
